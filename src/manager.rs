...
impl embedded_hal::i2c::I2c<SevenBitAddress> for &I2cManager {
    fn read(&mut self, addr: u8, buffer: &mut [u8]) -> Result<(), Self::Error> {
        self.i2c.lock().unwrap().read(addr, buffer, Duration::from_millis(TIMEOUT_MS).as_millis() as u32)
    }

    fn write(&mut self, addr: u8, buffer: &[u8]) -> Result<(), Self::Error> {
        self.i2c.lock().unwrap().write(addr, buffer, Duration::from_millis(TIMEOUT_MS).as_millis() as u32)
    }

    fn write_read(&mut self, addr: u8, wr_buffer: &[u8], rd_buffer: &mut [u8]) -> Result<(), Self::Error> {
        self.i2c.lock().unwrap().write_read(addr, wr_buffer, rd_buffer, Duration::from_millis(TIMEOUT_MS).as_millis() as u32)
    }

    fn transaction(&mut self, addr: u8, operations: &mut [Operation]) -> Result<(), Self::Error> {
        self.i2c.lock().unwrap().transaction(addr, operations, Duration::from_millis(TIMEOUT_MS).as_millis() as u32)
    }
}