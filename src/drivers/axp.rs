...
impl PowerManager {
    ...
    pub fn init_power(&mut self, i2c: &I2cManager) -> Result<()> {
        let mut axp = Axpxx::new(i2c);
        axp.init().map_err(|e| anyhow::anyhow!("{:?}", e))?;
        Ok(())
    }

    pub fn set_backlight(&mut self, i2c: &I2cManager, on: bool) -> Result<()> {
        let mut axp = Axpxx::new(i2c);
    ...
    pub fn get_battery_percentage(&mut self, i2c: &I2cManager) -> Result<u8> {
        let mut axp = Axpxx::new(i2c);
        axp.get_battery_percentage()
            .map_err(|e| anyhow::anyhow!("{:?}", e))
    }

    pub fn read_voltage(&mut self, i2c: &I2cManager) -> Result<u16> {
        let mut axp = Axpxx::new(i2c);
